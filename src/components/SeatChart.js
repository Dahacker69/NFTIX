...
const SeatChart = ({ occasion, nftixt, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await nftixt.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await nftixt.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
